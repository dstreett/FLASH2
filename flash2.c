...
"manually with the --min-overlap and --max-overlap options.",
	...
	*suffix = '\0';
	...
	if (verbose) {
		...
		info("    Min overlap outie:     %d",
		     alg_params.min_overlap_outie);
		info("    Max overlap:           %d",
		     alg_params.max_overlap);
	...
	if (verbose) {
		...
		/*David Streett Modification discarded Pairs*/
		uint64_t num_discarded;
		uint64_t num_of_reads;
		num_discarded = total_stats->num_discarded;
		...
		num_of_reads = num_total_pairs + num_discarded;
	
		info(" ");
		info("Read combination statistics:");
		info("    Total pairs:       %"PRIu64, num_of_reads);
		if (alg_params.discard_reads) {
			info("    Discarded pairs:   %"PRIu64, num_discarded);
			info("    Percent Discarded: %.2f%%",
		 	    TO_PERCENT(num_discarded, num_of_reads));
		}
